...
d.nombre AS departamneto
FROM empleados AS e
INNER JOIN sucursal AS s ON s.id = e.sucursalid
INNER JOIN municipio AS m ON m.id = s.municipioid
INNER JOIN departamento AS d ON d.id = m.depid;
```
...
p.nombre AS pais
FROM clientes AS c
INNER JOIN municipio AS m ON m.id = c.municipioid
INNER JOIN departamento AS d ON d.id = m.depid
INNER JOIN pais AS p ON p.id = d.paisid;
```
...
FROM sucursal AS s 
INNER JOIN empresa AS e ON e.id = s.empresaid
INNER JOIN municipio AS m ON m.id = s.municipioid
INNER JOIN departamento AS d ON d.id = m.depid
WHERE s.empresaid = 'EMP100';
```
...
JOIN municipio AS m ON m.id = s.municipioid
WHERE s.municipioid IN (
      SELECT COUNT(id) AS sucursales
      FROM sucursal
      HAVING sucursales > 1
);
```
...
FROM empresa AS e 
INNER JOIN sucursal AS s ON s.empresaid = e.id
INNER JOIN empleados AS em ON em.sucursalid = s.id
GROUP BY e.nombre;
```
...
FROM municipio AS m
INNER JOIN sucursal AS s ON s.municipioid = m.id
INNER JOIN empleados AS e ON e.sucursalid = s.id
GROUP BY m.nombre;
```
...
INNER JOIN municipio AS m ON m.depid = d.id
INNER JOIN clientes AS c ON c.municipioid = m.id
GROUP BY d.nombre
...
```sql
SELECT e.nombre AS empresa,
SUM(em.salario) AS total_salarios
FROM empresa AS e
INNER JOIN sucursal AS s ON s.empresaid = e.id
INNER JOIN empleados AS em ON em.sucursalid = s.id
GROUP BY e.nombre;
...
```sql
SELECT m.nombre AS municipio,
AVG(e.empleadoid) AS total_empleados
